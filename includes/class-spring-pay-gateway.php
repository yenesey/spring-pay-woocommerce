...
class Spring_Pay_Gateway extends WC_Payment_Gateway {
	...
	public function __construct() {
		...
		add_action('woocommerce_email_after_order_table', array( $this, 'insert_paylink'), 10, 4 );
	...
	*/
	...
	public function process_payment( $order_id ) {
		...
		// $spring_pay = sha1(strval($this->shop_id).strval($order_id)); референс для SpringPay
	...
	*/
	...
	public function pay_link($order_id) {
		$order = new WC_Order( $order_id );
		return 'https://t.me/SpringPayBot?start='.strval($this->shop_id).'_'.number_format($order->get_total(), 2, '', '').'_'.strval($order_id);
	}
	...
	function view_order( $order_id ) {
		...
		$image_name = 'qr_' . $order_id . '.png';
		$qr_file = $this->qr_image_dir . $image_name;
		$qr_url = $this->qr_image_url . $image_name;

		$pay_link = $this->pay_link($order_id);

		if ( !file_exists ($qr_file) ) {
			$this->QRcode->png($pay_link, $qr_file, 'L', 4, 2);
		}
		...
		if (!$mobile) {  
			$template .= '<li class="woocommerce-order-overview__order order">
				<center>
					<strong>
						<img src='.$qr_url.' alt="qrcode_place"/><span>Сканируйте код для оплаты смартфоном</span>
					</strong>
				</center>
			</li>';
		}
	...
	function insert_paylink( $order_id, $sent_to_admin, $plain_text, $email ) {
		/*
		...
		*/
		...
		echo '<h2>
				<a href="'.$this->pay_link($order_id).'">
					<button>Оплатить в SpringPay</button>
				</a>
			</h2>';
	}
